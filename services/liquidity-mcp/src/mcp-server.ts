...
const routeInputSchema = z.object({
  marketId: bytes32Schema.describe('ArcBook bytes32 market identifier'),
...
export function createLiquidityMcpServer(service: ExecutableLiquidityService): McpServer {
  const server = new McpServer({ name: 'arcbook-executable-liquidity', version: '1.0.0' }, {
    instructions: 'Use these read-only tools to discover ArcBook curve positions, obtain transparent routes, and compare Graph-backed liquidity. Never treat an indexed DEX estimate as executable calldata.',
  })

  server.registerTool('discover_positions', {
    title: 'Discover ArcBook positions',
    description: 'Discover active, backed maker curves for one market and side from the ArcBook Subgraph snapshot.',
  ...
  server.registerTool('quote_arcbook', {
    title: 'Quote ArcBook',
    description: 'Build a transparent unsigned multi-maker ArcBook quote after Graph discovery and bounded onchain refresh. This does not return a transaction as executable.',
  ...
  server.registerTool('build_candidate_route', {
    title: 'Build simulated ArcBook route',
  ...
  server.registerTool('compare_executable_liquidity', {
    ...
    description: 'Compare an onchain-simulated ArcBook route with a standardized DEX Subgraph snapshot. The response explicitly distinguishes executable calldata from indexed estimates.',
    inputSchema: routeInputSchema,
    annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
  ...
  server.registerResource(
    'arcbook-semantics',
    'arcbook://semantics',
    { title: 'ArcBook liquidity semantics', mimeType: 'text/markdown' },
    async (uri) => ({
      contents: [{
        uri: uri.href,
        mimeType: 'text/markdown',
        text: [
          '# ArcBook executable-liquidity semantics',
          '',
          ...
          '- ArcBook routes marked `onchain-simulated` passed BatchExecutor `eth_call` but remain unsigned.',
